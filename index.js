...
function setCORSHeaders(req, res) {
  ...
  res.setHeader(
    ...
    "Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, X-Signature, Access-Control-Request-Headers, Access-Control-Allow-Origin"
  );
...
app.post("/create-account", async (req, res) => {
  ...
  if (email) {
    const query = {
      text: "INSERT INTO account(id,email) VALUES($1,$2)",
      values: [id, email],
    };
...
app.put("/update-account", async (req, res) => {
  ...
  if (params.id && params.email && params.newEmail) {
    const query = {
      text: "UPDATE account SET email = $3 WHERE id = $1 AND email = $2;",
      values: [params.id, params.email, params.newEmail],
    };
...
app.delete("/delete-account", async (req, res) => {
  ...
  if (params.id && params.email) {
    const query = {
      text: "DELETE FROM account WHERE id = $1 AND email = $2;",
      values: [params.id, params.email],
    };
...
async function start() {
  ...
  const query = `CREATE TABLE "public"."account" ("id" character varying NOT NULL, "email" character varying NOT NULL, CONSTRAINT "account_email" UNIQUE ("email"), CONSTRAINT "account_id" PRIMARY KEY ("id")) WITH (oids = false);`;
